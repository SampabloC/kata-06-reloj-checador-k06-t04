...
module ReportsHelper
  ...
  def exist_reports_today(report)
    ...
    report if today_date == date_report
  end

  def exist_reports_month(report)
    today = Time.now
    date_report = report.check_in
    report if today.month == date_report.month && today.year == date_report.year
  end
  ...
  def average_check_in
    ...
    get_reports.each do |report|
      next unless report.check_in.month == Time.now.month && report.check_in.year == Time.now.year

      hours_check_in = report.check_in.strftime('%H').to_f
      minutes_check_in = report.check_in.strftime('%M').to_f

      hours_check_in += minutes_check_in / 60
